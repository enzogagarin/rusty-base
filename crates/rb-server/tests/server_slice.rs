...
fn serves_embedded_admin_ui_shell() {
    ...
    let collections_auth_js = app.handle(HttpRequest::new("GET", "/_/admin/collections/auth.js"));
    assert_eq!(collections_auth_js.status, 200);
    assert_eq!(
        collections_auth_js.content_type,
        "text/javascript; charset=utf-8"
    );
    ...
    let collections_import_export_js = app.handle(HttpRequest::new(
        "GET",
        "/_/admin/collections/import_export.js",
    ));
    assert_eq!(collections_import_export_js.status, 200);
    assert_eq!(
        collections_import_export_js.content_type,
        "text/javascript; charset=utf-8"
    );
    ...
    let settings_ui_js = app.handle(HttpRequest::new("GET", "/_/admin/settings_ui.js"));
    assert_eq!(settings_ui_js.status, 200);
    assert_eq!(
        settings_ui_js.content_type,
        "text/javascript; charset=utf-8"
    );
    let settings_ui_js = String::from_utf8(settings_ui_js.raw_body).unwrap();
    ...
    assert!(js_bundle.contains("collectionIsView"));
    assert!(js_bundle.contains("Read-only"));
    ...
    assert!(js_bundle.contains("collection-view-query-input"));
